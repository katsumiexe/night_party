$(function(){ 
	...
	$('.customer_detail').on('click','.tag_set',function () {
		if($(this).hasClass('tag_set_ck')!='true'){
			...
			$.post({
				url:"./post/customer_log_read"+Tmp+".php",
				data:{
					'c_id':C_Id,
				},
...
//		handle:'.customer_detail_box',
	...
	$('.body').on('click','.clist',function(){
		...
		$.post({
			...
			$.post({
				url:"./post/customer_log_read1.php",
				data:{
					'c_id'		:C_Id,
				},
			}).done(function(data2, textStatus, jqXHR){
				$('.customer_memo').html(data2);
...
//			$('.customer_sns_box').slideUp(1000);
			...
			$('.customer_sns_box').animate({'height':'0'},200);
...
//			$('.customer_sns_box').slideDown(2000);
			$('.customer_sns_box').animate({'height':'14vw'},200);
			$('.customer_sns_tr').slideDown(200);
			$('.customer_tag').animate({'margin-top':'91vw'},200);
