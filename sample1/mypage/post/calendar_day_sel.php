...
$dat=array();
//---------------------------------------------------------------
$sql	 ="SELECT stime,etime FROM ".TABLE_KEY."_schedule";
...
$dat["birth"]="";
$sql	 ="SELECT birth_day,id,name,nickname FROM ".TABLE_KEY."_customer";
$sql	.=" WHERE cast_id='{$cast_data["id"]}'";
...
if($result = mysqli_query($mysqli,$sql)){
	while($row = mysqli_fetch_assoc($result)){
		...
		$dat["birth"].="<span id=\"clist{$row["id"]}\" class=\"cal_days_birth_in clist\"><span class=\"days_icon\"></span><span class=\"days_birth\">{$row["nickname"]}({$tmp_age})</span></span>";
	}
}
