...
$sql	 ="SELECT P.id,P.date, P.view_date, P.title, P.log, P.cast, P.tag, P.img, P.img_del, P.status, P.prm , C.genji,C.cast_status FROM wp00000_posts AS P";
...
if($result = mysqli_query($mysqli,$sql)){
	while($res = mysqli_fetch_assoc($result)){
		...
		if($res["img"]){
			$res["img_link"]="../img/profile/{$res["cast"]}/{$res["img"]}.png?t={$res["prm"]}";
			$res["img_url"]="{$res["cast"]}/{$res["img"]}";
...
$sql	 ="SELECT COUNT(id) AS cnt FROM wp00000_posts";
$sql	.=" WHERE status<4";
$sql	.= $app;
