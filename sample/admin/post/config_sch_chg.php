...
if($val == 1){
	...
	$sql	 ="SELECT sort, id, name, time FROM `wp00000_sch_table` WHERE in_out='in' AND del=0 ORDER BY sort ASC";
	if($result = mysqli_query($mysqli,$sql)){
		while($row = mysqli_fetch_assoc($result)){

			if($row["sort"] < $admin_config["start_time"]){
	...
	foreach($dat as $a1 => $a2){
		$tmp_set++;
		if($a2["id"]){
			$sql	 ="UPDATE `wp00000_sch_table` SET sort={$tmp_set} WHERE id={$a2["id"]}";
			mysqli_query($mysqli,$sql);
			$tmp_auto	=$a2["id"];
			$tmp_name	=$a2["name"];
			$tmp_time	=$a1;
			...
			$sql	  ="INSERT INTO `wp00000_sch_table` (`in_out`,`sort`,`name`,`time`,`del`)";
	...
	$sql	 ="SELECT sort, id, name, time FROM `wp00000_sch_table` WHERE in_out='out' AND del=0 ORDER BY sort ASC";
	if($result = mysqli_query($mysqli,$sql)){
		while($row = mysqli_fetch_assoc($result)){
