...
$sql ="SELECT log_id, sdate, pts, nickname, name, days, customer_id FROM wp00000_cast_log AS A ";
...
if($result = mysqli_query($mysqli,$sql)){
	while($row = mysqli_fetch_assoc($result)){
		...
		$sql ="SELECT id, log_icon, log_color, log_comm, log_price FROM wp00000_cast_log_list";
...
//■カスタマーソート
$sql	 ="SELECT id, nickname,name,regist_date,birth_day,fav,c_group,face,tel,mail,twitter,insta,facebook,line,web,block, prm, MAX(L.date) AS h_date FROM wp00000_customer AS C";
		...
		<div class="cal_p_out">
			<table class="cal_table_p">
				...
				<tr>
					...
					$w=($s+$admin_config["start_week"]) % 7;
					?>
					<td class="cal_th_p <?=$week_tag[$w]?>"><?=$week[$n]?></td>
					<? } ?>
					<?=$cal_p[$n]?>
				</tr>
			</table>
...
<?}elseif($cast_page==2){?>
	<div class="main main_top">
		<div class="sub_header">
			...
			<div id="regist_customer_pc" class="regist_btn">
				<span class="regist_icon"></span>
				<span class="regist_txt">新規</span>
			</div>
		</div>
