...
if($c_s ==0){
	...
	$sql	 ="SELECT id, staff_id, name, kana, mail, tel, C.del, genji, cast_status, `group` FROM ".TABLE_KEY."_staff AS S";
...
$(function(){ 
	...
	$('#staff_sort').on('change','.box_sort',function(){
		$('#wait').show();
		Base_id=$(this).parent(".w40").parent(".tr").attr('id').replace('sort_item','');
...
<tbody id="staff_sort" class="sort_main_staff">
<?for($n=0;$n<$count_dat;$n++){?>
	<?if($dat[$n]["view"] == 1){?>
		<?$pg_list++;?>
		<?if($pg_list>=$pg_st && $pg_list<=$pg_ed){?>
			<tr id="sort_item<?=$dat[$n]["staff_id"]?>" class="tr b<?=$dat[$n]["cast_status"]?>">
				<td class="td_sort handle w40"></td>
				<td class="w40"><input id="s_box<?=$dat[$n]["cast_sort"]?>" type="text" value="<?=$pg_list?>" class="box_sort"></td>
		...
		<?}?>
	<?}else{?>
		<tr id="sort_item<?=$dat[$n]["staff_id"]?>" class="tr" style="display:none;">
			<td></td>
			<td></td>
			<td></td>
			<td></td>
...
<td class="td_top w80">グループ</td>
<td class="td_top w140">メールアドレス</td>
<td class="td_top w80">所属</td>
<td class="td_top w140">キャスト名</td>
<td class="td_top w140">ステータス</td>
<td class="td_top w60">変更</td>
</tr>
</thead>
<tbody class="sort_main">
<?for($n=0;$n<$count_dat;$n++){?>
<tr class="tr b<?=$dat[$n]["cast_status"]?>">
	...
	<td class="td_staff"><?=$dat[$n]["genji"]?></td>
	<td class="td_staff"><?=$cast_status_select[$dat[$n]["cast_status"]]?></td>
