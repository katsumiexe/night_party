...
//■キャスト件数カウント
$sql ="SELECT cast, genji, COUNT(".TABLE_KEY."_posts.id) AS cnt ,MAX(view_date) AS b_date FROM ".TABLE_KEY."_posts";
$sql.=" LEFT JOIN ".TABLE_KEY."_cast ON ".TABLE_KEY."_posts.cast=".TABLE_KEY."_cast.id";
$sql.=" WHERE status=0";
$sql.=" AND cast_status=0";
$sql.=" AND view_date<='{$now}'";
$sql.=" GROUP BY cast";
...
if($res = mysqli_query($mysqli,$sql)){
	while($a1 = mysqli_fetch_assoc($res)){
		...
		if (file_exists("./img/profile/{$a1["cast"]}/0_s.webp") && $admin_config["webp_select"] == 1) {
...
//■カテゴリ件数カウント
$sql ="SELECT tag_name, tag_icon, ".TABLE_KEY."_tag.id, COUNT(".TABLE_KEY."_posts.id) AS cnt FROM ".TABLE_KEY."_tag";
$sql.=" LEFT JOIN ".TABLE_KEY."_posts ON ".TABLE_KEY."_tag.id=".TABLE_KEY."_posts.tag";
$sql.=" LEFT JOIN ".TABLE_KEY."_cast ON ".TABLE_KEY."_posts.cast=".TABLE_KEY."_cast.id";
$sql.=" WHERE tag_group='blog'";
$sql.=" AND (status=0 OR status IS NULL)";
$sql.=" AND (view_date<='{$now}' OR view_date IS NULL)";
$sql.=" AND cast_status=0";
$sql.=" AND  ".TABLE_KEY."_tag.del=0";
$sql.=" GROUP BY ".TABLE_KEY."_tag.id";
...
$sql ="SELECT P.id,view_date, title, img, img_del, cast, genji,tag_name,tag_icon FROM ".TABLE_KEY."_posts AS P";
$sql.=" LEFT JOIN ".TABLE_KEY."_cast AS C ON P.cast=C.id";
...
if($result = mysqli_query($mysqli,$sql)){
	while($row = mysqli_fetch_assoc($result)){
		...
		if (file_exists("./img/profile/{$row["cast"]}/0.webp") && $admin_config["webp_select"] == 1) {
...
<div class="main_top_flex">
	...
	<div class="main_flex_b">
		...
		<div class="sub_blog">
			<?if($tag_count){?>
			<article class="sub_blog_in">
				<h3 class="blog_h1">カテゴリー</h3>
